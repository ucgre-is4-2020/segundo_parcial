...
<body>
	...
	<table>
		...
		<tr>
			<td><a href="{{ route('tp2-ug0093-ug0278-ug0307-ver-direccion-empresa',
			 ['id' => $undireccionempresa->id]) }}"
			 title="Mostrar datos de Dirección de la Empresa">{{ $undireccionempresa->id }}</a></td>
			<td>{{ $undireccionempresa->empresa->razon_social }}</td>
			<td>{{ $undireccionempresa->barrio->nombre }}</td>
			<td>{{ $undireccionempresa->direccion_empresa_tipo->nombre }}</td>
			<td>{{ $undireccionempresa->calle }}</td>
			<td>{{is_numeric($undireccionempresa->numero)?number_format($undireccionempresa->numero,0,',','.'):$undireccionempresa->numero}}</td>
			<td>{{ $undireccionempresa->latitud }}</td>
			<td>{{ $undireccionempresa->longitud }}</td>
			@if ($undireccionempresa->es_casa_central)
				<td>{{ "Es casa central" }}</td>
			@else
				<td>{{ "No es casa central" }}</td>
			@endif 
			<td>{{ $undireccionempresa->nombre_ubicacion }}</td>
			@if ($undireccionempresa->es_deposito)
				<td>{{ "Es depósito" }}</td>
			@else
				<td>{{ "No es depósito" }}</td>
			@endif 
			<td>{{ $undireccionempresa->created_at }}</td>
			<td>{{ $undireccionempresa->updated_at }}</td>
			<td><a class="accion editar" 
				href="{{ route('tp2-ug0093-ug0278-ug0307-editar-direccion-empresa', ['id' => $undireccionempresa->id]) }}"
				title="Editar Registro">Editar</a>
			</td>
			<td><a class="accion borrar" 
				href="{{ route('tp2-ug0093-ug0278-ug0307-confirmar-borrar-direccion-empresa', ['id' => $undireccionempresa->id]) }}"
	...
	<div class="fondo">
		<div class="center">		
			...
			<a href="{{ route('tp2-ug0093-ug0278-ug0307-crear-direccion-empresa') }}" title="Crear Registro">Crear Registro</a>
			
			<form action="/tp2/ug0093-ug0278-ug0307/listado-direccion-empresa" method="GET">
				<input class="buscar" id="buscar" name="buscar" type="text" 
				placeholder="Buscar por calle..." value="{{ empty($busqueda)?'':$busqueda }}">
				<button class="btn-buscar" type="submit">Buscar</button>
				<a href="{{ route('tp2-ug0093-ug0278-ug0307-listado-direccion-empresa') }}" class="btn-buscar" >Limpiar</a>
			</form>
